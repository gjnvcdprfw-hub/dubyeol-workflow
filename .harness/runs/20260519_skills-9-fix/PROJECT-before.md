...
## r7 Addendum — 회고 1~36 영구화 및 sync 정책 결정 기록 (2026-05-17 KST)

본 addendum은 PROJECT.md의 기존 회고 본문을 삭제·덮어쓰지 않고, r7 정비 task에서 회고 1~36을 다음 세대 운영 매뉴얼에 반영하기 위한 상위 결정으로 기록한다. 본 세션에서 새로 발생하는 회고는 r37부터 번호를 부여한다.

### r7-A. 회고 반영 원칙

| 원칙 | 내용 |
|---|---|
| 역사 보존 | 회고 1~36 본문은 삭제·덮어쓰기하지 않는다. |
| addendum 방식 | 정정은 대상 문서의 addendum 또는 후속 결정 기록으로 추가한다. |
| 결함 기반 정정 | 잘 돌아가는 영역은 손대지 않고, 회고로 확인된 결함·혼선만 보강한다. |
| client 보존 | silkroadhub 사업 자산과 키 파일은 마스터 운영 task에서 보존 대상으로 둔다. |

### r7-B. 마스터·클라이언트 sync 정책 보강 (회고 28·31)

마스터는 두별 워크플로우 원본·매뉴얼·스킬·run 이력을 관리하고, 클라이언트는 sync 정책에 따라 필요한 운영 자산을 받아 적용한다. client-local 키 로드 파일이나 사업 자산을 마스터 task의 참조·복사·수정 대상으로 삼지 않는다. OpenAI 등 외부 키는 시스템 환경변수 사용을 원칙으로 하며, 키 값 출력은 금지한다.

### r7-C. 본 세션 결정 이력 addendum

| 날짜 | 결정 | 사유 | 영향 모듈 |
|---|---|---|---|
| 2026-05-17 | Task 3 residue 4건을 r7 정비로 이관한다. | 해당 residue는 9개 스킬 실행 코드가 아니라 references/README 안내 문서에 해당하므로 Task 3 scope를 확대하지 않고 r7 문서 정비로 처리한다. | §C.1 |
| 2026-05-17 | 본 세션 신규 회고는 r37부터 시작한다. | 회고 1~36은 역사 기록으로 보존하고, 새 회고 번호 충돌을 방지한다. | §D |
